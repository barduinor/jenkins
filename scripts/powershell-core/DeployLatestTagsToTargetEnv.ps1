...
# Load CSV from file
#$csv_env = import-csv -Path /tmp/LT.Environments.csv.mapping
#$csv_app = import-csv -Path /tmp/LT.Applications.csv.mapping
$csv_env = import-csv -Path $env:WORKSPACE/LT.Environments.csv.mapping
$csv_app = import-csv -Path $env:WORKSPACE/LT.Applications.csv.mapping
...
$apps = "Azure Cognitive Services Connector,Barcode Plugin,Business Activity Monitoring" -split ","
#$apps = $env:ApplicationsToDeploy

$AppKeys = ""
foreach($item in $apps){
    $Appkeys = $Appkeys + "," + ($csv_app.Where({$_.Name -eq $item}).Key)
    }
$AppKeys = $AppKeys.subsstring(1)
